...
async def locks_func(client: Client, message: Message):
    ...
    if parameter in data:
        ...
        await message.edit_text(f"🔒 Locked all permission in {message.chat.title}")
        ...
        await message.edit(f"🔓 Unlocked all permission in {message.chat.title}")
...
add_command_help(
    "locks",
    [
        ["lock <all or parameters>", "Lock permissions on the group."],
        [
            "unlock <all or parameters>",
            "Open permissions on the group\n\nSupported Locks / Unlocks:` `msg` | `media` | `stickers` | `polls` | `info`  | `invite` | `webprev` |`pin` | `all`.",
        ],
    ],
)
