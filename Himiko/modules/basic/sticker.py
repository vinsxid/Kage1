...
from Himiko.helpers.tools import get_arg, get_text, resize_media
from Himiko.utils.tools import add_text_img, bash
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await Man.edit("**The sticker has no Names.**")
                return
            ...
            await Man.edit("**Unsupported Files.**")
            return
        media_ = await client.download_media(replied, file_name="Himiko/resources/")
    else:
        await Man.edit("**Reply to Media.**")
        return
    if media_:
        ...
        if exist is not False:
            ...
            if (
                ...
                await Man.edit(
                    "**Failed to Add Sticker, Use @Stickers To Add Your Sticker.**"
                )
                return
            await client.send_message("Stickers", emoji_)
            await asyncio.sleep(2)
            ...
            if (
                ...
                await Man.edit(
                    "**Failed to Add Sticker, Use @Stickers To Add Sticker You.**"
                )
                return
            await client.send_message("Stickers", emoji_)
            await asyncio.sleep(2)
            ...
            await asyncio.sleep(2)
        await Man.edit(
            f"**Successfully Added** **[Click Here](https://t.me/addstickers/{packname})**"
        )
...
async def packinfo(client: Client, message: Message):
    ...
    if not message.reply_to_message:
        await rep.edit("Reply To Sticker...")
        return
...
async def cb_sticker(client: Client, message: Message):
    ...
    if not query:
        return await message.reply("**Enter the name of the Sticker Pack.**")
...
async def memify(client: Client, message: Message):
    ...
    if not reply_message.media:
        await message.text("**Reply to stickers!**")
        return
    file = await client.download_media(reply_message)
    mm = await message.reply_text("`Processing . . .`")
    text = get_arg(message)
    if len(text) < 1:
        return await mm.edit("Type `.mmf text`")
...
async def tinying(client: Client, message: Message):
    ...
    im1 = Image.open("Himiko/resources/blank.png")
    if ik.endswith(".tgs"):
